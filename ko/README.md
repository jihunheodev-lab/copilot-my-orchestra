...
- **파이프라인 오케스트레이션**: Research -> Plan -> Implement -> Test -> Review 흐름 내장
- **자율/가이드 모드**: 서브에이전트 자동 위임과 사용자 주도 핸드오프 모두 지원
...
| 환경 | Orchestrator(@) | 서브에이전트 위임 | 핸드오프 버튼 |
|-------------|-------------------|---------------------|-----------------|
| VS Code Insiders | 지원 | 지원 | 지원 |
| VS Code Stable | 지원 | 부분 지원 | 지원 |
| GitHub.com | 지원 | 미지원 | 미지원 |
| Copilot CLI | 지원 | 미지원 | 미지원 |

## 기본 파이프라인

시스템의 기본 흐름은 다음 5단계입니다.
1. **Research**: 기존 구현/의존성 분석
2. **Plan**: 기술 명세와 단계별 작업 분해
3. **Implement**: 작은 단위의 원자적 코드 변경 실행
4. **Test**: 단위/통합/회귀 테스트 검증
5. **Review**: 성능/보안/프로젝트 규칙 최종 점검
